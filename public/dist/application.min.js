...
"use strict";angular.module("pictures").controller("PicturesController",["$scope","$stateParams","$location","Authentication","Pictures","$http",function($scope,$stateParams,$location,Authentication,Pictures,$http){$scope.authentication=Authentication,$scope.picture=null,$scope.flickrPictureId=0,$scope.linkToHost="",$scope.flickrSuccess=!0,$scope.sizes=[],$scope.getSourceBySize=function(sizeLabel,picture){for(var key in picture.sizes)if(picture.sizes[key].label===sizeLabel)return picture.sizes[key].source},$scope.fetchFlickrData=function(){null!==$scope.picture&&($scope.linkToHost=$scope.picture.linkToHost),$scope.flickrSuccess=!0,$scope.error="",$scope.flickrSquareIsAvailable=!1,$scope.flickrSmallIsAvailable=!1,$scope.flickrMediumIsAvailable=!1,$scope.flickrLargeIsAvailable=!1,$scope.flickrOriginalIsAvailable=!1,$scope.sizes=[];var flickrPictureId=checkFlickrBBcode($scope.linkToHost),host="https://api.flickr.com",path="/services/rest/",method="flickr.photos.getSizes",apiKey="e52b35ac4e8f7389ec81840ff2497704",format="json&nojsoncallback=1",fullUrl=host.concat(path,"?method=",method,"&api_key=",apiKey,"&photo_id=",flickrPictureId,"&format=",format);$http.get(fullUrl).success(function(data,status){if("ok"===data.stat){var index=0;for(index=0;index<data.sizes.size.length;index++)switch($scope.sizes.push({label:data.sizes.size[index].label,source:data.sizes.size[index].source,width:data.sizes.size[index].width,height:data.sizes.size[index].height}),data.sizes.size[index].label){case"Square":$scope.flickrSquareSource=data.sizes.size[index].source,""!==$scope.flickrSquareSource&&($scope.flickrSquareIsAvailable=!0);break;case"Small":$scope.flickrSmallSource=data.sizes.size[index].source,""!==$scope.flickrSmallSource&&($scope.flickrSmallIsAvailable=!0);break;case"Medium 640":$scope.flickrMediumSource=data.sizes.size[index].source,""!==$scope.flickrMediumSource&&($scope.flickrMediumIsAvailable=!0);break;case"Large":$scope.flickrLargeSource=data.sizes.size[index].source,""!==$scope.flickrLargeSource&&($scope.flickrLargeIsAvailable=!0);break;case"Original":$scope.flickrOriginalSource=data.sizes.size[index].source,""!==$scope.flickrOriginalSource&&($scope.flickrOriginalIsAvailable=!0)}$scope.flickrSuccess=!1}else $scope.error=data.message||"Request failed";data=null,status=null}).error(function(data){$scope.error=data||"Request failed"})};var checkFlickrBBcode=function(flickrUrl){for(var flickrUrlId=flickrUrl.replace("https://flic.kr/p/",""),alphabet="123456789abcdefghijkmnopqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ",num=flickrUrlId.length,decoded=0,multi=1,i=num-1;i>=0;i--)decoded+=multi*alphabet.indexOf(flickrUrlId[i]),multi*=alphabet.length;return decoded};$scope.create=function(){var picture=new Pictures({name:this.name,description:this.description,linkToHost:this.linkToHost,sizes:$scope.sizes});picture.$save(function(){$location.path("pictures"),$scope.name=""},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(picture){if(picture){picture.$remove();for(var i in $scope.pictures)$scope.pictures[i]===picture&&$scope.pictures.splice(i,1)}else $scope.picture.$remove(function(){$location.path("pictures")})},$scope.update=function(){var picture=$scope.picture;picture.sizes=$scope.sizes,picture.$update(function(){$location.path("pictures/list")},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){$scope.pictures=Pictures.query(initializeSlides)};var initializeSlides=function(){var i,j,slides=$scope.slides=[];for(i=0;i<$scope.pictures.length;i++){var slideAttributes=[];for(slideAttributes.name=$scope.pictures[i].name,j=0;j<$scope.pictures[i].sizes.length;j++){{$scope.pictures[i].description}"Large"===$scope.pictures[i].sizes[j].label?slideAttributes.largeImg=$scope.pictures[i].sizes[j].source:"Medium 640"===$scope.pictures[i].sizes[j].label?slideAttributes.mediumImg=$scope.pictures[i].sizes[j].source:"Square"===$scope.pictures[i].sizes[j].label&&(slideAttributes.squareImg=$scope.pictures[i].sizes[j].source)}slides.push(slideAttributes)}};$scope.findOne=function(){$scope.picture=Pictures.get({pictureId:$stateParams.pictureId})}}]);

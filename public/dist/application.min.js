"use strict";var ApplicationConfiguration=function(){var applicationModuleName="MichaelBritsch",applicationModuleVendorDependencies=["ngResource","ui.router","ui.bootstrap","ui.utils","angularFileUpload","ngTouch","slick"],registerModule=function(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)};return{applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:applicationModuleVendorDependencies,registerModule:registerModule}}();
...
"use strict";angular.module("appointments").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Referenzen",state:"appointments",type:"dropdown"}),Menus.addSubMenuItem("topbar","appointments",{title:"Referenzen anzeigen",state:"appointments.list"}),Menus.addSubMenuItem("topbar","appointments",{title:"Referenz anlegen",state:"appointments.create"})}]);
...
"use strict";angular.module("core").controller("HeaderController",["$scope","$state","Authentication","Menus",function($scope,$state,Authentication,Menus){$scope.$state=$state,$scope.authentication=Authentication,$scope.userIsAdmin=!1,console.log($scope.authentication),$scope.menu=Menus.getMenu("topbar"),$scope.isCollapsed=!1,$scope.toggleCollapsibleMenu=function(){$scope.isCollapsed=!$scope.isCollapsed},$scope.$on("$stateChangeSuccess",function(){$scope.isCollapsed=!1}),$scope.userIsAdmin=!1,$scope.checkUserRoles=function(){var i;if(""!==$scope.authentication.user){for(i=0;i<$scope.authentication.user.roles.length;i++)if("admin"===$scope.authentication.user.roles[i])return void($scope.userIsAdmin=!0)}else $scope.userIsAdmin=!1}}]);
...
"use strict";angular.module("pictures").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Bilder",isPublic:!0,state:"pictures",type:"dropdown"}),Menus.addSubMenuItem("topbar","pictures",{title:"Bilder anzeigen",isPublic:!0,state:"pictures.list"}),Menus.addSubMenuItem("topbar","pictures",{title:"Bild anlegen",isPublic:!1,roles:["admin","user"],state:"pictures.create"})}]);

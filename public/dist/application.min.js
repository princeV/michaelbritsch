"use strict";var ApplicationConfiguration=function(){var applicationModuleName="MichaelBritsch",applicationModuleVendorDependencies=["vnHelper","ngResource","ui.router","ui.bootstrap","ui.utils","angularFileUpload","ngTouch","slick","textAngular"],registerModule=function(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)};return{applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:applicationModuleVendorDependencies,registerModule:registerModule}}();
...
"use strict";angular.module("contacts").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Kontakt",isPublic:!0,state:"contact.view",position:4})}]);
"use strict";angular.module("contacts").config(["$stateProvider",function($stateProvider){$stateProvider.state("contact",{"abstract":!0,url:"/kontakt",template:"<ui-view/>"}).state("contact.view",{url:"",templateUrl:"modules/contacts/views/view-contact.client.view.html"}).state("contact.create",{url:"/anlegen",templateUrl:"modules/contacts/views/create-contact.client.view.html"}).state("contact.edit",{url:"/:contactId/bearbeiten",templateUrl:"modules/contacts/views/edit-contact.client.view.html"})}]);
"use strict";angular.module("contacts").controller("ContactsController",["$scope","$stateParams","$http","$location","Authentication","Contacts",function($scope,$stateParams,$http,$location,Authentication,Contacts){$scope.authentication=Authentication,$scope.form={name:"",subject:"",mailBody:""},$scope.create=function(){var contact=new Contacts({firstName:this.firstName,lastName:this.lastName,street:this.street,postalCode:this.postalCode,telephone:this.telephone,mobile:this.mobile,fax:this.fax,city:this.city,email:this.email});contact.$save(function(){$location.path("/kontakt"),$scope.firstName="",$scope.lastName="",$scope.street="",$scope.postalCode="",$scope.telephone="",$scope.mobile="",$scope.fax="",$scope.city="",$scope.email=""},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(contact){if(contact){contact.$remove();for(var i in $scope.contacts)$scope.contacts[i]===contact&&$scope.contacts.splice(i,1)}else $scope.contact.$remove(function(){$location.path("/")})},$scope.update=function(){var contact=$scope.contact;contact.changed=new Date,contact.$update(function(){$location.path("kontakt")},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.findFirst=function(){$scope.contacts=Contacts.query(function(contactResults){$scope.contact=contactResults[0]})},$scope.findOne=function(){$scope.contact=Contacts.get({contactId:$stateParams.contactId})},$scope.mailSuccess=$scope.mailError=null,$scope.disableForm=!1,$scope.initialiseContactForm=function(){$scope.form={name:"",subject:"",mailBody:""},$scope.mailSuccess=$scope.mailError=null,$scope.disableForm=!1},$scope.askForContactMail=function(){$scope.success=$scope.error=null;var name=$scope.authentication.user.displayName||$scope.form.name,email=$scope.authentication.user.email||"n/a",mailBodyUser=$scope.form.mailBody.replace(new RegExp("\r?\n","g"),"<br />")+"<br /><br />******************************************************<br />Von: "+name+"<br />Email: "+email+"<br />******************************************************",eMailMessage={subject:$scope.form.subject,mailBody:mailBodyUser};$http.post("/api/contactform/send",eMailMessage).success(function(response){response.error?($scope.mailError=response.message,$scope.disableForm=!0):($scope.mailSuccess=response.message,$scope.disableForm=!0)}).error(function(response){$scope.mailError=response.message,$scope.disableForm=!0})}}]);
...
"use strict";angular.module("biographies").config(["$stateProvider",function($stateProvider){$stateProvider.state("biographies",{"abstract":!0,url:"/biografie",template:"<ui-view/>"}).state("biographies.create",{url:"/anlegen",templateUrl:"modules/biographies/views/create-biography.client.view.html"}).state("biographies.view",{url:"",templateUrl:"modules/biographies/views/view-biography.client.view.html"}).state("biographies.edit",{url:"/:biographyId/bearbeiten",templateUrl:"modules/biographies/views/edit-biography.client.view.html"})}]);
"use strict";angular.module("biographies").controller("BiographiesController",["$scope","$stateParams","$location","Authentication","Biographies",function($scope,$stateParams,$location,Authentication,Biographies){$scope.authentication=Authentication,$scope.create=function(){var biography=new Biographies({name:this.name,htmlContent:this.htmlContent});biography.$save(function(){$location.path("biografie"),$scope.name="",$scope.htmlContent=""},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(biography){if(biography){biography.$remove();for(var i in $scope.biographies)$scope.biographies[i]===biography&&$scope.biographies.splice(i,1)}else $scope.biography.$remove(function(){$location.path("/")})},$scope.update=function(){var biography=$scope.biography;biography.changed=new Date,biography.$update(function(){$location.path("biografie")},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.findFirst=function(){$scope.biographies=Biographies.query(function(biographyResults){$scope.biography=biographyResults[0]})},$scope.find=function(){$scope.biographies=Biographies.query()},$scope.findOne=function(){$scope.biography=Biographies.get({biographyId:$stateParams.biographyId})}}]);
...
"use strict";angular.module("vnHelper",[]).directive("vnFirstCharStrong",["$window",function(){return{restrict:"E",scope:{inputText:"@"},transclude:!0,template:"<h2><strong>{{firstChar}}</strong>{{remainingChars}}</h2><ng-transclude></ng-transclude>",link:function(scope,elem,attrs){attrs.$observe("inputText",function(value){void 0!==value&&(scope.firstChar=value.charAt(0),scope.remainingChars=value.substring(1))})}}}]);
...
"use strict";angular.module("pictures").controller("PicturesController",["$scope","$stateParams","$location","Authentication","Pictures","$http",function($scope,$stateParams,$location,Authentication,Pictures){$scope.authentication=Authentication,$scope.sizes=[],$scope.setPictureIndex=function(index){$scope.pictureIndex=index},$scope.slick={current:0,init:function(index){this.current=index},"goto":function(index){this.current=index},next:function(){this.current++},prev:function(){this.current--},onBeforeChange:function(){},onAfterChange:function(){}},$scope.getSourceBySize=function(sizeLabel,picture){for(var key in picture.sizes)if(picture.sizes[key].label===sizeLabel)return picture.sizes[key].source},$scope.remove=function(picture){if(picture){picture.$remove();for(var i in $scope.pictures)$scope.pictures[i]===picture&&$scope.pictures.splice(i,1)}else $scope.picture.$remove(function(){$location.path("pictures")})},$scope.update=function(){var picture=$scope.picture;picture.$update(function(){$location.path("pictures/list")},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){Pictures.query(function(pictureResults){$scope.pictures=pictureResults})},$scope.findOne=function(){$scope.picture=Pictures.get({pictureId:$stateParams.pictureId})}}]);
"use strict";angular.module("pictures").controller("PictureImgUploadController",["$scope","$timeout","$location","$window","Pictures","Authentication","FileUploader",function($scope,$timeout,$location,$window,Pictures,Authentication,FileUploader){$scope.authentication=Authentication,$scope.loading=!1,$scope.uploaderUrl="api/pictures",$scope.canvasObject={},$scope.uploader=new FileUploader({url:$scope.uploaderUrl}),$scope.uploader.queueLimit=1,$scope.uploader.filters.push({name:"imageFilter",fn:function(item){var type="|"+item.type.slice(item.type.lastIndexOf("/")+1)+"|";return-1!=="|jpg|png|jpeg|".indexOf(type)}}),$scope.uploader.onAfterAddingFile=function(fileItem){$scope.canvasObject={file:fileItem._file,width:1024,height:768,previewWidth:150,previewHeight:150},$scope.addingFileError=null},$scope.uploader.onWhenAddingFileFailed=function(){$scope.addingFileError="Das Bild muss den Typ .jpg oder .png haben."},$scope.uploader.onSuccessItem=function(fileItem,response){$scope.loading=!1,$scope.picture=response,$scope.uploader.clearQueue(),$location.path("pictures")},$scope.uploader.onErrorItem=function(fileItem,response){$scope.loading=!1,$scope.uploader.clearQueue(),$scope.error=response.message},$scope.uploader.onBeforeUploadItem=function(fileItem){fileItem._file=$scope.canvasObject.file},$scope.startUpload=function(){$scope.error=null,$scope.uploader.queue.length<1?$scope.error="Bitte ein Bild "+decodeURI("ausw%C3%A4hlen"):($scope.uploader.queue[0].formData.push({name:$scope.name,description:$scope.description}),$scope.loading=!0,$scope.uploader.uploadAll())}}]);
"use strict";angular.module("pictures").directive("picturePreview",["$window",function(){return{restrict:"A",template:"<canvas/>",scope:{picturePreview:"="},link:function(scope,element){function onLoadImage(){var ctx=canvas.getContext("2d"),sourceImage=this,imageHeight=sourceImage.height,imageWidth=sourceImage.width,maxTargetImageSize=calculateMaxImageSize(scope.picturePreview.width,scope.picturePreview.height,imageWidth,imageHeight);canvas.width=maxTargetImageSize.width,canvas.height=maxTargetImageSize.height,ctx.drawImage(sourceImage,0,0,imageWidth,imageHeight,0,0,maxTargetImageSize.width,maxTargetImageSize.height);for(var mimeType=scope.picturePreview.file.type,dataURL=canvas.toDataURL(mimeType),dataURLHeader="data:"+mimeType+";base64,",data=atob(dataURL.substring(dataURLHeader.length)),asArray=new Uint8Array(data.length),i=0,len=data.length;len>i;++i)asArray[i]=data.charCodeAt(i);var blob=new Blob([asArray.buffer],{type:mimeType}),maxPreviewImageSize=calculateMaxImageSize(scope.picturePreview.previewWidth,scope.picturePreview.previewHeight,imageWidth,imageHeight);canvas.width=maxPreviewImageSize.width,canvas.height=maxPreviewImageSize.height,ctx.drawImage(sourceImage,0,0,maxPreviewImageSize.width,maxPreviewImageSize.height),blob.lastModifiedDate=new Date,blob.name=scope.picturePreview.file.name,scope.picturePreview.file=blob}function onLoadFile(event){var img=new Image;img.onload=onLoadImage,img.src=event.target.result}function calculateMaxImageSize(maxWidth,maxHeight,imageWidth,imageHeight){var aspectRatio=maxHeight/maxWidth,imgageAspectRatio=imageHeight/imageWidth,targetwidth=maxWidth,targetheight=maxHeight;imgageAspectRatio>aspectRatio?targetheight=targetwidth/imageWidth*imageHeight:targetwidth=targetheight/imageHeight*imageWidth;var maxImageSize={width:targetwidth,height:targetheight};return maxImageSize}var canvas=element.find("canvas")[0],reader=new FileReader;reader.onload=onLoadFile,reader.readAsDataURL(scope.picturePreview.file)}}}]);
...
"use strict";angular.module("videos").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Videos",isPublic:!0,state:"videos.slick",position:2})}]);
